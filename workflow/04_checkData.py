...
# check Null for userId in set log_clean
df_sorted = df_log.cube('timestamp').count() \
    .filter('timestamp NOT LIKE \'NONE\'') \
    .filter('timestamp NOT LIKE \'Adjustment\'') \
    .orderBy(['count', 'timestamp'], ascending=[False, True])
df_sorted.show(5, False)
print('Not Null formated_ts with filter: %d '% df_log.where(col("timestamp").isNotNull()).count())
print('Null with formated_ts filter: %d '% df_log.where(col("timestamp").isNull()).count())
